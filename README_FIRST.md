...
1. 科研复现和比赛演示推荐运行 `NeuroEphysAI-Setup-1.2.5-Full.exe`，并保留默认的“完整 GPU/Kilosort”组件；没有兼容 NVIDIA GPU 或只做 CPU 分析时选择更小的 `NeuroEphysAI-Setup-1.2.5.exe`。Full 安装页也可以取消 GPU 组件。安装过程不需要管理员权限。
...
```powershell
python -m venv .venv
.\.venv\Scripts\python.exe -m pip install neuroephys_ai-1.2.5-py3-none-any.whl
.\.venv\Scripts\neuroephys.exe info
```
...
Run `NeuroEphysAI-Setup-1.2.5.exe`, then open **NeuroEphys AI** from the desktop
