# NeuroEphys AI 1.3.3 — 首次使用 / First read

## Windows 普通用户

1. 日常 CPU 分析请运行 `NeuroEphysAI-Setup-1.3.3.exe`。需要完整 GPU/CUDA/Kilosort 离线依赖时，使用 `NeuroEphysAI-Setup-1.3.3-Full.exe`；Full 安装页也可以取消 GPU 组件。安装过程不需要管理员权限。
...
```powershell
python -m venv .venv
.\.venv\Scripts\python.exe -m pip install neuroephys_ai-1.3.3-py3-none-any.whl
.\.venv\Scripts\neuroephys.exe info
```
...
Run `NeuroEphysAI-Setup-1.3.3.exe`, then open **NeuroEphys AI** from the desktop
