...
1. 推荐运行 `NeuroEphysAI-Setup-1.2.1.exe`（或含 GPU 依赖的 Full 包），安装过程不需要管理员权限。
...
```powershell
python -m venv .venv
.\.venv\Scripts\python.exe -m pip install neuroephys_ai-1.2.1-py3-none-any.whl
.\.venv\Scripts\neuroephys.exe info
```
...
Run `NeuroEphysAI-Setup-1.2.1.exe`, then open **NeuroEphys AI** from the desktop
