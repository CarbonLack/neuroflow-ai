...
|---|---|---|---|
| **Full offline installer** `NeuroEphysAI-Setup-1.4.1-Full.exe` | Competition demos, research workstations, Kilosort/GPU use | Run the installer and choose components | Includes all v1.4.1 fixes plus the complete offline GPU/CUDA/Kilosort component |
| **Standard installer (recommended)** `NeuroEphysAI-Setup-1.4.1.exe` | General Windows, teaching, CPU workflows | Run the installer | Includes the latest AI conversation and chart-vision entry points; optional sorter components can be added later |
| **Standard portable ZIP** `NeuroEphysAI-1.4.1-Windows-x64-portable.zip` | Computers without install permission | Extract everything, then run `NeuroEphysAI\NeuroEphysAI.exe` | Do not copy only the EXE |
| **Python wheel** `neuroephys_ai-1.4.1-py3-none-any.whl` | Scripts, batch processing, and API users | Install with `python -m pip` | Python 3.12 is recommended |
