...
TUTORIAL_DETAILS: dict[str, dict[str, Any]] = {
    ...
    "unit_qc": {
        ...
        "operations": [
            ...
            _operation(
                "逐次波形、均值与同接点 PCA",
                "Individual waveforms, mean, and same-contact PCA",
                "先看每次 spike 的原始波形云，再看均值、中位数和 10–90 百分位；核对显示数/总数。切换接点及 PC1–PC3 轴，点击散点选中 Unit；PCA 纳入该接点全部候选 cluster，但大数据只抽样显示点。",
                "Inspect individual raw spikes, then the mean, median, and 10th–90th percentile; check shown/total counts. Change contact and PC1–PC3 axes, and click points to select a Unit. PCA includes every candidate cluster on that contact, while large spike sets are sampled for display.",
                "均值概括典型形态，散布揭示变异或异常；PCA 分离仅是复核线索。通道几何未知时，不把数字相邻通道当作空间相邻。",
                "The mean summarizes typical shape while the cloud reveals variability and outliers; PCA separation is only review evidence. Numeric channel neighbors are not spatial neighbors without geometry.",
                "保留波形、特征空间与人工结论的可追溯证据；不自动宣称单细胞真值。",
                "Retains traceable waveform and feature evidence without claiming single-cell ground truth.",
            ),
            _operation(
                "ACG 与 ISI",
                "ACG and ISI",
    ...
    "analysis": {
        ...
        "operations": [
            _operation(
                "人工筛选集与行为事件",
                "Curated cohort and behavior events",
                "先在 Unit 质控应用已复核候选单神经元，再点击“选择行为事件 / Tuning”选择 1 或 2 个行为。",
                "Apply reviewed candidate single units in Unit QC, then choose one or two behaviors in Select behavior events / Tuning.",
                "让每次 tuning、统计和解码使用明确的 Unit 清单与行为事件，而不是自动取前两个条件。",
                "Makes each tuning, statistics and decoding run use an explicit Unit cohort and event selection instead of the first two conditions.",
                "保存筛选 revision、Unit IDs、requested conditions、实际事件数和排除原因；标签变化会阻止旧筛选集继续运行。",
                "Saves cohort revision, Unit IDs, requested conditions, retained events and exclusions; label changes block use of a stale cohort.",
            ),
            _operation(
                "完整神经活动分析包",
