...
输入框按 **回车发送**，按 **Shift+回车换行**。无需打开项目也能询问一般科研知识或其他问题；打开项目后，助手会将一般知识、软件教程与本项目实测证据区分。询问具体软件操作时，它可以检索版本化的内置教程。没有联网查证能力时，它不应把时效性信息或未核实文献说成已经验证。未注册的新分析仍需先增加参数、前提与权限定义；AI 不会自动获得任意 Python 或文件系统权限。
