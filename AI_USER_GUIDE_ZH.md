...
对话自动保存到项目的 `ai/conversation.json`；重开同一项目可继续，历史检索不跨项目。查询证据记录具体工具、参数、实际读数和快照时间。长对话的近期内容直接随请求提供，更早内容通过关键词检索，不是无限上下文。

AI 读取图标题、坐标范围及结构化结果，不代表它已经看见图像像素。原始电压和 ground truth 不由该查询接口提供。未注册的新分析需先增加参数、前提与权限定义；不会自动获得任意 Python 或文件系统权限。
