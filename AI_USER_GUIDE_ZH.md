...
3. v1.3.3 会选择 Harness SDK 连接，由已安装的官方 dsh 负责身份认证和模型请求。App 不读取、复制 Harness 的模型密钥，不绕过机构规定直接访问接口。
...
对话按项目分成多个会话，自动用首个问题命名，也可以在展开窗口中重命名；可将会话分到“项目分析、图表解读、方法问答、通用问题、早期对话”，用当前图发起的新会话自动归入图表解读。“查找”会搜索分类、标题和问答正文。旧版平铺对话会保留在“早期对话”，不会丢失。这是 App 内部分类，不会更改外部 Harness 网页的“未分组”。项目内会话与问答自动保存到 `ai/conversation.json`；重开同一项目可继续，历史检索不跨项目。查询证据记录具体工具、参数、实际读数和快照时间。每次只把当前会话的近期问答直接带入模型，更早的项目对话可通过关键词查询；不同会话不会混成一条对话。

默认只读取图标题、坐标范围及结构化结果，**不代表它已经看见图像像素**。需要图像解读时，使用右侧“解读图”或展开窗口“解读当前图”：App 仅渲染当前图为 PNG，在发送前显示预览并再次征得确认，随后通过官方 Harness SDK 图像消息块发送。模型可以讨论可见趋势，但不能从像素冒充精确统计；数值结论应再查项目结果。PNG 字节不存入对话档案，档案只保留图名与发送记录。图中可能含原始波形或标签，请在预览时自行检查。当前仅 Harness SDK 图像入口可用；普通兼容 API Provider 不会悄悄降级为“假装已看图”。
