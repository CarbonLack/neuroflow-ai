...
def test_release_workflow_does_not_hardcode_an_old_version():
    ...
    assert f"body_path: docs/RELEASE_DOWNLOAD_{release_series}_ZH.md" in workflow
    ...
    assert '-not $env:GITHUB_ACTIONS' in release_script
    assert 'Skipping workstation-only living archive refresh' in release_script
