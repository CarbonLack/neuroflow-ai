...
- 正式版下载：https://github.com/CarbonLack/neuroflow-ai/releases
...
- Release downloads: https://github.com/CarbonLack/neuroflow-ai/releases
...
def assemble(destination: Path, private_project: Path | None = None) -> None:
    ...
    if source_release.exists():
        for source in sorted(source_release.iterdir()):
            if source.is_file() and copy_existing(source, release_dir / source.name):
                package_names.append(source.name)
    ...
    write_index(destination, tuple(package_names))
