...
def main():
    ...
    sections = []
    batches = [('四极电极模拟：20分钟', 'tetrode'),
               ('双探针模拟：修正几何后的20分钟', 'geometry_corrected/neuropixels')]
    batches += [(f'真实记录：{path.name}', path.relative_to(root).as_posix())
                for path in sorted((root / 'real').glob('subject*')) if path.is_dir()]
    for label, relative in batches:
    ...
    page += '<h1>NeuroEphys AI · 实验结果导航</h1><p class="notice">这是工作中交付索引，不是全部完成证明。每种工具独立保存；候选簇、自动筛选和人工验收不是一回事。真实记录按确认的通道组独立分析，身份未知的组不得与行为配对。旧的重叠探针几何结果仅保留作失败诊断。</p>'
    if (root / '9月16日进度与质量说明.md').exists():
        page += '<p>先读 <a href="9月16日进度与质量说明.md">进度与质量说明</a>。</p>'
    page += '<p>下方报告包含英文图件、图注草稿和中文解释。App中使用“导入项目”，选择列出的 neuroflow_project.json。</p>'
    page += ''.join(sections) + '<p>此导航不替代质量验收清单。原始数据不修改，私有结果不上传公开GitHub。</p></html>'
